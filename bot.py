...
# qr_module.py
"""
QR module for telegram bot.
Usage:
    from qr_module import register_qr_handlers, init_mongo_client

    client = init_mongo_client(MONGO_URI)  # or pass an existing pymongo.MongoClient
    register_qr_handlers(app, mongo_client=client, db_name="qr_bot",
                         collection_name="commands", admins_collection="admins",
                         currency="INR", super_admin_id=123456789)
"""
...
# ----------------- Helpers -----------------
def init_mongo_client(mongo_uri: Optional[str]):
    """Return a MongoClient. If None, raises ValueError."""
    if not mongo_uri:
        raise ValueError("MONGO_URI is required for QR module")
    return MongoClient(mongo_uri)

def build_upi_link(amount: Decimal, upi_id: str, currency: str = "INR") -> str:
    am_str = f"{amount:.2f}"
...
# ----------------- Module state -----------------
# We'll store DB references when register_qr_handlers is called
_db = None
_collection = None
_admins_col = None
_currency = "INR"
...
def is_admin(user_id: int) -> bool:
    ...
    if _admins_col is None:
        return False
    return _admins_col.find_one({"user_id": user_id}) is not None

# ----------------- Commands -----------------
async def start_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "Hi! I'm a QR generator module 🤖\n\n"
        "Commands:\n"
        "/save {unique_command} {upi_id} - Save your UPI command (Admins only).\n"
        "Then use: /<command> <amount>\n\n"
        "Use /helpqr to see all QR commands."
    )

async def help_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "📖 QR Commands:\n\n"
        "/save {command} {upi_id} - Save a new command (Admins only)\n"
        "/listqr - Show saved QR commands\n"
        "/deleteqr {command} - Delete a command (Owner only)\n\n"
        "Usage example:\n"
        "/save food paytm@upi\n"
        "Then use: /food 100"
    )
...
async def addadmin_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    _admins_col.update_one(
        {"user_id": user.id},
        {"$set": {"username": user.username}},
        upsert=True
    )
    await update.message.reply_text(f"✅ @{user.username} added as admin successfully!")
...
async def removeadmin_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    result = _admins_col.delete_one({"user_id": user.id})
    if result.deleted_count > 0:
        await update.message.reply_text(f"✅ @{user.username} removed from admins.")
    else:
        await update.message.reply_text(f"❌ @{user.username} is not an admin.")

async def save_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_admin(update.effective_user.id):
        await update.message.reply_text("❌ Only owner or added admins can use this command.")
        return

    if len(context.args) != 2:
        await update.message.reply_text(
            "Usage: /save {unique_command} {upi_id}\nExample: /save aqr afzal@ybl"
        )
        return
    ...
    if _collection.find_one({"command": unique_command}):
...
async def list_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = "📋 Saved Commands:\n"
    for c in cmds:
        msg += f"/{c['command']} → {c['upi_id']}\n"
    await update.message.reply_text(msg)

async def delete_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_owner(update.effective_user.id):   # only owner allowed
        await update.message.reply_text("❌ Only the bot owner can delete commands.")
        return

    if len(context.args) != 1:
        await update.message.reply_text("Usage: /deleteqr {command}")
        return

    cmd_to_delete = context.args[0].strip().lower()
    record = _collection.find_one({"command": cmd_to_delete})
    if not record:
        await update.message.reply_text("❌ Command not found.")
        return

    _collection.delete_one({"command": cmd_to_delete})
    await update.message.reply_text(f"✅ Command /{cmd_to_delete} deleted successfully.")
...
async def dynamic_qr_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    data = _collection.find_one({"command": cmd})
    if not data:
        return  # Not a saved command
    ...
    upi_id = data["upi_id"]
    upi_link = build_upi_link(amt, upi_id, currency=_currency)
    qr_bytes = make_qr_png_bytes(upi_link)

    if update.message.reply_to_message:
        replied_text = update.message.reply_to_message.text or ""
        buyer_match = re.search(r"BUYER\s*:\s*(.+)", replied_text, re.IGNORECASE)
        buyer = buyer_match.group(1).strip() if buyer_match else "buyer"
        ...
        caption = (
            f"Please {buyer} pay and send ss here ⤵️\n\n"
            f"💸UPI: {upi_id}\n"
            f"💰Amount: ₹{amt}\n"
            f"💰Scan to pay exact ₹{amt}"
        )
        await update.message.reply_to_message.reply_photo(photo=qr_bytes, caption=caption)
    else:
        caption = f"UPI: `{upi_id}`\nAmount: ₹{amt:.2f}\nScan to pay."
        await update.message.reply_photo(photo=qr_bytes, caption=caption, parse_mode="Markdown")

# ----------------- Registration -----------------
def register_qr_handlers(app: Application,
    ...
    """
    Register QR handlers on an existing Application instance.
    Provide either mongo_client or mongo_uri.
    """
    global _db, _collection, _admins_col, _currency, _SUPER_ADMIN

    if mongo_client is None:
        if not mongo_uri:
            raise ValueError("Provide mongo_client or mongo_uri")
        mongo_client = init_mongo_client(mongo_uri)

    _db = mongo_client[db_name]
    _collection = _db[collection_name]
    _admins_col = _db[admins_collection]
    _currency = currency
    _SUPER_ADMIN = super_admin_id
